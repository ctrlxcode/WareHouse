...
<div class="row">
     <div class="col-lg-12">
             <h1 class="page-header"><i class="fa fa-users fa-fw"></i> สมาชิก</h1>
     </div>        
</div>
...
<?= GridView::widget([
    ...
    'columns' => [
    	[
        	'attribute' => 'สถานะ',
        	'contentOptions' => ['align' => 'middle'],
        	'value' => function($model){
        		$sql="SELECT s.user_id FROM user u LEFT JOIN session s on s.user_id = u.id";
        		$users = Yii::$app->db->createCommand($sql)->queryAll();
        		foreach($users as $user){
        			if($user['user_id'] == $model->id){
        				$status = "<span class='label label-success'>ออนไลน์</span>";
        				return $status;
        			} else if($user['user_id'] == NULL){
        				$status = "<span class='label label-danger'>ออฟไลน์</span>";
        				return $status;
        			}
        		} 
